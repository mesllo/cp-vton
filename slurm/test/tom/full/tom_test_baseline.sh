...
#SBATCH --output=slurm/test/tom/full/output/tom_test_baseline.out
...
PLF_LAYERS=SHALLOW      # choose from SHALLOW, MID, DEEP, ALL, DEFAULT
PLN=vgg19
DATASET=viton
...
TOM_DIR="$STAGE"_train_"$RELEVANCE_LEVEL"_id"$ID"_finetune"$FINETUNE_CONFIG"_layers"$PLF_LAYERS"_"$PLN"_"$DATASET"_"$GMM_MODEL"
