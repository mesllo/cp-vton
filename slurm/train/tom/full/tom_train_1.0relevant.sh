...
#SBATCH --output=slurm/train/tom/full/output/tom_train_1.0relevant_warpmod3_3.out
...
FINETUNE_CONFIG=ALL     # choose from ALL, MID, DEEP, NONE
...
PLN_PATH=finetune_ImageNet_"$RELEVANCE_LEVEL"relevant_"$PLN"_finetune"$FINETUNE_CONFIG"_epochs"$EPOCHS"_lr"$LEARNING_RATE"_patience"$PATIENCE"_decay"$DECAY"_batch"$BATCH_SIZE"_"$WEIGHT_INIT".pth.tar
...
time python train.py --pretrained --dataroot "$TMPDIR"/bonelloj/"$DATASET" --name $EXP_NAME --stage $STAGE --datamode $MODE --data_list $PAIRS --warp_dir $WARP_DIR --workers 4 --save_count $SAVE_COUNT --shuffle --keep_step $KEEP_STEP --decay_step $DECAY_STEP --rel $RELEVANCE_LEVEL --fin $FINETUNE_CONFIG --plf $PLF_LAYERS --pln $PLN --pln_path $PLN_PATH --dataset $DATASET
